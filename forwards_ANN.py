...
y_fwd = forwards.pop('ln_mktval')


# dataframe split
X_train, X_test, y_train, y_test = train_test_split(forwards, y_fwd, test_size=0.2, random_state=42)
sc = StandardScaler()
...
history = model.fit(X_train, y_train, epochs=n_epochs, batch_size=batch_size, callbacks=[callback],
...
mae = np.mean(losses.mean_absolute_error(y_test, y_pred))
mse = np.mean(losses.mean_squared_error(y_test, y_pred))

r2 = RSquare(num_regressors=X_test.shape[1])
...
results.to_excel('fwd_predANN.xlsx')


# plt.plot(y_test, y_pred, '.')
# plt.show()

# plt.plot(y_test_MIL, y_pred_MIL, '.')
# plt.show()
